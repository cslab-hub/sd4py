...
def bootstrapping(
    ...
    if isinstance(subgroups, list):
            ...
            def get_indices(sel):
                ...
                if isinstance(sel, sd4py.PyNumericSelector):
                    if sel.include_lower_bound and sel.lower_bound != float("-inf"):
                        np.logical_and(
                            logical_indices,
                            sample[sel.attribute].values >= sel.lower_bound,
                            out=logical_indices,
                        )  ## It's about x10 faster to use .values (i.e. numpy arrays and therefore numpy functions)
                    ...
                    if sel.include_upper_bound and sel.upper_bound != float("inf"):
                        np.logical_and(
                            logical_indices,
                            sample[sel.attribute].values <= sel.upper_bound,
                            out=logical_indices,
                        )
                ...
                if isinstance(sel, sd4py.PyNominalSelector):
                    np.logical_and(
                        logical_indices,
                        sample[sel.attribute].astype(str).values == sel.value,
                        out=logical_indices,
                    )
        ...
        samples_indices = dict(
            zip(range(number_simulations), map(sample_indices, samples))
        )
        ...
        if aggregation_function is not None:
            aggregation = {
                key: aggregation_function(val) for key, val in results.items()
            }
...
def confidence_intervals(
    ...
    if (
        data.loc[:, target].dtype == "object"
        or data.loc[:, target].dtype == "bool"
        or data.loc[:, target].dtype.name == "category"
    ):  ## if nominal
        ...
        def metric_function(sample, subgroup_sample):
            ...
            target_proportion = (
                subgroup_sample.eq(subgroups.target_value).sum()
                / subgroup_sample.count()
            )  ## what proportion of values is equal to the target value
    ...
    def aggregation_function(subgroup_values):
        out = {
            "proportion_lower": np.nanquantile(
                [val[0] for val in subgroup_values], 0.05
            ),
            "proportion_upper": np.nanquantile(
                [val[0] for val in subgroup_values], 0.95
            ),
    ...
    bootstrapping_results, confidence_intervals = bootstrapping(
        subgroups,
        data,
        metric_function,
    ...
    return bootstrapping_results, pd.DataFrame(
        {"pattern": str(subgroup), **values}
        for subgroup, values in confidence_intervals.items()
    )
...
def confidence_precision_recall_f1(
    subgroups,
    data,
    ignore_defaults=False,
    number_simulations=100,
    ...
    def metric_function(sample, subgroup_sample):
        ...
        precision = (
            subgroup_sample.values.__eq__(target_value).sum() / subgroup_sample.count()
        )  ## Use numpy arrays to check for equality since they're much faster
        recall = (
            subgroup_sample.values.__eq__(target_value).sum()
            / sample.values.__eq__(target_value).sum()
        )  ## Use numpy arrays to check for equality since they're much faster
    ...
    def aggregation_function(subgroup_values):
        out = {
            "precision_lower": np.nanquantile(
                [val[0] for val in subgroup_values], 0.05
            ),
            "precision_upper": np.nanquantile(
                [val[0] for val in subgroup_values], 0.95
            ),
    ...
    bootstrapping_results, aggregation = bootstrapping(
        subgroups,
        data,
        metric_function,
        aggregation_function,
    ...
    return bootstrapping_results, pd.DataFrame(
        {"pattern": str(subgroup), **values} for subgroup, values in aggregation.items()
    )
...
def corrected_hedges_g(sample1, sample2):
    ...
    pooled_sd = np.sqrt(
        (((n_1 - 1) * sample1.var()) + ((n_2 - 1) * sample2.var())) / (n_1 + n_2 - 2)
    )
...
def odds_ratio_ci(sample1, sample2):
    ...
    lower = np.exp(
        np.log(odds_ratio) - (1.96 * np.sqrt((1 / a) + (1 / b) + (1 / c) + (1 / d)))
    )
    upper = np.exp(
        np.log(odds_ratio) + (1.96 * np.sqrt((1 / a) + (1 / b) + (1 / c) + (1 / d)))
    )
...
def find_interesting_columns(
    subgroup, data, use_complement=True, ignore_defaults=False, columns_to_ignore=[]
):
    """
    ...
    for column in data:
        ...
        ):  ## these need to be converted to a straightforward numeric format
            column = pd.to_numeric(column)
            column = (
                column - column.mean()
            ) / column.std()  ## Just to get to a reasonable timescale, otherwise it's nanoseconds or something like that
        ...
        if (
            column.dtype == "object"
            or column.dtype == "bool"
            or column.dtype.name == "category"
        ):  ## nominals
            vals, counts = np.unique(column.dropna(), return_counts=True)

            for value in vals[np.argsort(-counts)][
                :5
            ]:  ## 5 most common values for this variable; each providing a feature-value pair to investigate
                nominal_columns[(column.name, value)] = odds_ratio_ci(
                    subgroup_rows == value, population_rows == value
                )

        else:  ## numerics
            numeric_columns[column.name] = corrected_hedges_g(
                subgroup_rows, population_rows
            )
...
def most_interesting_columns(subgroup, data, columns_to_ignore=[]):
    ...
    interesting_numeric, interesting_nominal = find_interesting_columns(
        subgroup, data, columns_to_ignore=columns_to_ignore
    )

    interesting_numeric = pd.DataFrame(
        interesting_numeric.values(), index=interesting_numeric.keys()
    )
    interesting_numeric = interesting_numeric.dropna()
    interesting_nominal = pd.DataFrame(
        interesting_nominal.values(), index=interesting_nominal.keys()
    )
    interesting_nominal = interesting_nominal.dropna()

    if len(interesting_numeric) > 0:
        interesting_numeric = interesting_numeric.dropna()
        most_interesting_numeric = interesting_numeric.iloc[
            interesting_numeric[0].abs().argsort()
        ][::-1][0].iloc[
            :10
        ]  ## Find the 10 most interesting by effect size

    else:
        most_interesting_numeric = interesting_numeric

    if len(interesting_nominal) > 0:
        max_lower = interesting_nominal.loc[
            interesting_nominal[1].abs().groupby(level=0).idxmax()
        ][
            1
        ]  ## Maximum lower confidence interval
        max_lower = max_lower.iloc[max_lower.abs().argsort()][::-1]
        max_lower

        min_upper = interesting_nominal.loc[
            interesting_nominal[2].abs().groupby(level=0).idxmin()
        ][2]
        min_upper = min_upper[
            interesting_nominal.groupby(level=0).count()[2].values > 2
        ]
        ...
        most_interesting_nominal = (
            pd.concat([max_lower, min_upper]).sort_values(ascending=False).iloc[:10]
        )  ## 10 most interesting by having especially high or especially low odds ratio
...
def radar_plot(
    ...
    ax.yaxis.set_visible(
        False
    )  # Axes and appropriate scales will be drawn later, using polar_twin
    ...
    for idx, x_tick in enumerate(x_ticks[0]):
        x_tick.set_pad(
            (axis_padding // 5) + (axis_padding * np.abs(np.sin(angles[idx]) ** 2))
        )  ## This seems to give slightly better padding than the previous attempt

    def polar_twin(
        ax, ymin, ymax, angle=20
    ):  # function to make a new axis with appropriate tick marks
        ...
        ax2.set_rgrids(
            [1, 1 + (prop_scale / 3), 1 + (2 * prop_scale / 3), 1 + prop_scale],
            labels,
            angle,
            size=text_size,
            ha="center",
    ...
    for idx, colname in enumerate(data):
        ...
        ax_latest.set_zorder(
            100
        )  # so axis grid doesn't appear in front of other content
    ...
    def plot_polygon(row, angles, colour, label, linestyle):
        ...
        ax.plot(
            angles, values, linewidth=2, linestyle=linestyle, color=colour, label=label
        )
        ax.fill(angles, values, colour, alpha=0.1)

    data_norm = 1 + (
        prop_scale * (data - ymins) / (ymaxes - ymins)
    )  # Scale the data to match to the labels
    ...
    for idx, row in enumerate(data_norm.values):
        ...
        plot_polygon(
            row,
            angles,
            list(mcolors.TABLEAU_COLORS)[idx % len(mcolors.TABLEAU_COLORS)],
            label=str(data_norm.index[idx]),
            linestyle=["solid", "dashed", "dotted", "dashdot"][idx % 4],
        )
...
def subgroup_overview(
    ...
    def visualise_columns(
        ...
        if numeric_columns is not None:
            ...
            if use_complement:
                means = pd.concat(
                    [
                        subgroup_means,
                        visualisation_data.drop(subgroup_indices, axis=0)[
                            numeric_columns
                        ].mean(numeric_only=False),
                    ],
                    axis=1,
                ).T.set_index([["Subgroup", "Complement"]])
                ...
                means = pd.concat(
                    ...
                    axis=1,
                ).T.set_index([["Subgroup", "Population"]])

            vis_data_numerics = visualisation_data[numeric_columns]

            numeric_ymins = vis_data_numerics.mean(numeric_only=False) - (
                vis_data_numerics.std(numeric_only=False)
            )

            numeric_ymins = pd.concat([numeric_ymins, subgroup_means], axis=1).T.min(
                numeric_only=False
        ...
        if nominal_columns is not None:
            ...
            subgroup_proportions = (
                nominal_data.loc[subgroup_indices, :].eq(nominal_values).sum()
            ...
            if use_complement:
                proportions = pd.concat(
                    [
                        subgroup_proportions,
                        nominal_data.drop(subgroup_indices, axis=0)
                        .eq(nominal_values)
                        .sum()
            ...
            nominal_ymins = (
                (2 * proportions) - 1
            ).min()  ## same distance below the proportion as above it (up to 1), minimum across subgroup and complement
            nominal_ymins = pd.concat(
                [nominal_ymins, pd.Series(0, index=nominal_ymins.index)], axis=1
            ).T.max()  ## set to zero if currently below zero

            nominal_ymaxes = (
                2 * proportions
            ).max()  ## same distance above the proportion as below it (down to 1), maximum across subgroup and complement
            nominal_ymaxes = pd.concat(
                [nominal_ymaxes, pd.Series(1, index=nominal_ymaxes.index)], axis=1
            ).T.min()  ## set to 1 if currently above 1

            nominal_ymins.index = [
                "{0} == {1}".format(*x) for x in zip(nominal_columns, nominal_values)
            ]
            nominal_ymaxes.index = [
                "{0} == {1}".format(*x) for x in zip(nominal_columns, nominal_values)
            ]
            proportions.columns = [
                "{0} == {1}".format(*x) for x in zip(nominal_columns, nominal_values)
    ...
    if (
        visualisation_data[target].dtype == "object"
        or visualisation_data[target].dtype == "bool"
        or visualisation_data[target].dtype.name == "category"
    ):
        ## For nominal target, use a stacked barchart to visualise distribution

        target_data = visualisation_data[target][
            ~visualisation_data[target].isna()
        ]  # get rid of NaN
        ...
        if use_complement:
            pd.concat(
                [
                    pd.Series(
                        *np.unique(
                            target_data.loc[subgroup_indices].dropna(),
                            return_counts=True,
                        )[::-1],
                        name="Subgroup"
                    )
                    ...
                    pd.Series(
                        *np.unique(
                            target_data.drop(subgroup_indices, axis=0).dropna(),
                            return_counts=True,
                        )[::-1],
                        name="Complement"
                    )
                ...
                axis=1,
            ).T.plot(kind="barh", stacked=True, cmap=plt.get_cmap("Set2"), ax=ax)
            ...
            pd.concat(
                [
                    ...
                    / target_data.loc[subgroup_indices].count(),
                    pd.Series(
                        *np.unique(target_data.dropna(), return_counts=True)[::-1],
                        name="Complement"
                    )
                    / target_data.count(),
                ],
        ...
        if use_complement:
            sns.kdeplot(
                visualisation_data.loc[subgroup_indices][target],
                linewidth=2,
                label="Subgroup",
            )
            sns.kdeplot(
                visualisation_data.drop(subgroup_indices, axis=0)[target],
                linewidth=2,
                label="Complement",
                linestyle="dashed",
            )
    ...
    ax = visualise_columns(
        numeric_columns=numeric_selectors,
        nominal_columns=nominal_selectors,
        nominal_values=nominal_selector_values,
        subplot=222,
    )
    ...
    columns_to_ignore = [
        s.attribute for s in subgroup.selectors
    ]  ## Selectors will already be visualised
    columns_to_ignore += [target]  ## Target will already be visualised

    most_interesting_numeric, most_interesting_nominal = most_interesting_columns(
        subgroup, selection_data, columns_to_ignore=columns_to_ignore
    )
    ...
    if len(most_interesting_numeric) > 0:
        ax = visualise_columns(
            numeric_columns=most_interesting_numeric.index.tolist(), subplot=223
        )
    ...
    if len(most_interesting_nominal) > 0:
        ...
        ax = visualise_columns(
            nominal_columns=columns, nominal_values=values, subplot=224
        )
    ...
    for idx1, subgroup1 in enumerate(subgroups):
        for idx2, subgroup2 in enumerate(subgroups):
            if idx1 < idx2:
                ...
                adjacency[idx1, idx2] = (
                    indices1.intersection(indices2).size / indices1.union(indices2).size
                )

    G = nx.from_numpy_array(adjacency * (adjacency > minimum_jaccard))
...
def subgroup_background_rectangle(ax, subgroup, features, window_size):
    ...
    for i, index_value in enumerate(features.index):
        ...
        start = (
            i * window_size
        ) - 1 / 2  ## -1/2 so first point of the window is included in the rectangle
        end = (
            (i + 1) * window_size
        ) - 1 / 2  ## -1/2, otherwise an extra point is included in the rectangle
...
def time_plot(subgroup, features, *series, window_size, use_start=False):
    ...
    def get_xtick_formatter_function(largest_timedelta, smallest_timedelta):
        ...
        largest_component = np.arange(7)[
            np.array(largest_timedelta.components, dtype=bool)
        ][0]
        smallest_component = np.arange(7)[
            np.array(smallest_timedelta.components, dtype=bool)
        ][-1]
    ...
    assert [len(x) for x in series].count(len(series[0])) == len(
        series
    ), "Input series are not the same length"  ## Check all input series are the same length
    ...
    for idx, series in enumerate(series):
        ...
        if isinstance(series, pd.DataFrame):
            name = (
                series.columns.name
            )  ## This can be set with df.rename_axis('New Name', axis=1)
        else:
            name = series.name
        ...
        ax.set_xticks(
            ax.get_xticks().tolist()[1:-1]
        )  # First and last xticks always seem irrelevant

        subgroup_background_rectangle(ax, subgroup, features, window_size)

        if use_start:
            ax.set_xticklabels(
                [((x * timestep_timedelta) + start) for x in ax.get_xticks().tolist()],
                size=12,
            )
        else:
            ax.set_xticklabels(
                [formatter(x * timestep_timedelta) for x in ax.get_xticks().tolist()],
                size=12,
            )
